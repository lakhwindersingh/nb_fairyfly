...
The **Percipience Cloud SaaS Portal & Corporate Platform** provides the commercial web interface connecting public brand marketing, customer self-service onboarding, usage-based billing, and real-time context observability:
...
3. **Usage-Based Metering & Rev-Share Engine**: Stripe billing for base subscriptions ($1,499 Team, $4,499 Business, $9,999 Enterprise) and automated 15% token savings revenue-share calculation.
4. **Mission-Control Observability Dashboard**: Real-time token burn velocity, prompt cache hit ratios, active worktrees, Merkle DAG transitions, and one-click rollbacks.
5. **Shared-to-Decoupled Infrastructure**: Initial shared hosting with Play 3 core infrastructure keeping OpEx $< \$350/\text{month}$, with zero-refactoring decoupling capability.
...
│   │   ├── mod_saas_portal/     # Multi-tenant customer dashboard & control plane
...
- **Sprint 2**: Multi-tenant auth (SSO/SAML), workspace onboarding wizard, and PostgreSQL RLS.
- **Sprint 3**: Stripe billing webhooks and automated 15% token savings rev-share ledger.
- **Sprint 4**: 5-Tab Observability Hub integration, real-time WebSocket feeds, and E2E Playwright tests.
