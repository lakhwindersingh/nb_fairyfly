...
This document is a **Layerable Domain-Specific Context Engineering Plan** designed to overlay onto the generic **Parent Master Context Engineering Framework**. It injects web frontend systems, SaaS customer portals, billing gateways, and real-time observability telemetry dashboards:
...
3. **Decoupled API Gateways & Billing Handlers**: Stripe billing webhooks and automated 15% token savings revenue-share calculation.
4. **Observability Hub Dashboard**: Live WebSocket and REST telemetry feed (`user/outputs/dashboard/index.html`).
...
graph TD
  ...
  subgraph API_Gateway_Layer["API Gateway & Services"]
    PortalServer["Portal Gateway Server"]
...
- `.nb/context/contracts/`: `onboarding_contract.yaml`, `billing_meter_contract.yaml`, `observability_contract.yaml`
- `.nb/context/rules/`: `web_accessibility_rules.md`, `tenant_isolation_invariants.md`
- `.nb/agentic/custom/agents/`: `agent_saas_portal_architect.yaml`, `agent_billing_integration_engineer.yaml`, `agent_observability_frontend_specialist.yaml`
- `workplace/modules/`: `mod_corp_site/`, `mod_saas_portal/`, `mod_billing_engine/`, `mod_api_gateway/`
- `user/outputs/`: Maturity reports and live dashboard feeds.
